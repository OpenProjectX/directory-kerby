...
KERBY_KDC_ALLOW_TCP="${KERBY_KDC_ALLOW_TCP:-true}"
KERBY_KDC_ALLOW_UDP="${KERBY_KDC_ALLOW_UDP:-false}"
...
[kdcdefaults]
  ...
  kdc_allow_tcp = ${KERBY_KDC_ALLOW_TCP}
  kdc_allow_udp = ${KERBY_KDC_ALLOW_UDP}
...
EOF
if [ "${KERBY_KDC_ALLOW_UDP}" = "true" ]; then
  cat >> "${KERBY_CONF_DIR}/kdc.conf" <<EOF
...
dump_debug_configuration() {
  ...
  echo "KERBY_KDC_ALLOW_TCP=${KERBY_KDC_ALLOW_TCP}"
  echo "KERBY_KDC_ALLOW_UDP=${KERBY_KDC_ALLOW_UDP}"
