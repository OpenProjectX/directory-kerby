...
KERBY_KDC_ALLOW_TCP=true
KERBY_KDC_ALLOW_UDP=false
...
The image defaults to TCP-only KDC startup with `KERBY_KDC_ALLOW_UDP=false`.
This avoids Kerby `KadminTool` loading an ambiguous classpath resource named
`krb5_udp.conf` during container bootstrap. Java clients generated by this
image already prefer TCP.
